...
- **Resharing an official source.** Credibility travels with the *origin*, not the resharer — so a post that **links** an official-allowlist domain (an NWS advisory URL) inherits official treatment and isn't flagged even from an unverified account. This is the *link* form of resharing; the *quote-post* form (quoting another account) is future work, as it requires the ingestion to capture Bluesky's `post.embed` provenance.
